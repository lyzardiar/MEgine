...
test('MCP resource subscriptions coalesce bridge events and stop after unsubscribe', async () => {
  ...
  assert.deepEqual(
    EVENT_RESOURCE_URIS['window.types.changed'],
    ['mengine://editor/window/types'],
  );
  assert.deepEqual(
    EVENT_RESOURCE_URIS['menu.changed'],
    ['mengine://editor/menus'],
  );
  assert.deepEqual(
    EVENT_RESOURCE_URIS['build.progress'],
    ['mengine://build/status', 'mengine://build/artifact'],
  );
  assert.deepEqual(
    EVENT_RESOURCE_URIS['build.artifacts'],
    ['mengine://build/history', 'mengine://build/patches'],
  );
  assert.deepEqual(
    EVENT_RESOURCE_URIS['asset.changed'],
    [
