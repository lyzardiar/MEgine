...
export function applyAspectRatio(
  ...
  mode: string,
  aspectRatio: number,
  drivenAxes: [boolean, boolean] = [false, false],
): Rect {
  const ratio = Number(aspectRatio);
  if (mode === 'None' || !Number.isFinite(ratio) || ratio <= 0) return { ...rect };
  if (drivenAxes[0] && drivenAxes[1]) return { ...rect };
  const px = rect.x + rect.w * pivot[0];
  const py = rect.y + rect.h * (1 - pivot[1]);
  if (mode === 'WidthControlsHeight') {
    if (drivenAxes[1]) return { ...rect };
    const h = rect.w / ratio;
    return { x: rect.x, y: py - h * (1 - pivot[1]), w: rect.w, h };
  }
  if (mode === 'HeightControlsWidth') {
    if (drivenAxes[0]) return { ...rect };
  ...
  return {
    ...
    y: parent.y + (parent.h - h) * (1 - pivot[1]),
    w,
    h,
  };
}
