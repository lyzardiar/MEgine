...
export const QUERY_PARAMS_SCHEMAS: Record<string, AgentJsonSchema> = {
  ...
  'scene.authored_snapshot': emptySchema,
  'scene.diff': objectSchema({
    fromRevision: {
      type: 'integer',
      minimum: 0,
      description: 'Active-scene revision returned by scene.snapshot or a previous scene.diff result',
    },
  }, ['fromRevision']),
  ...
  'view.capture_region': objectSchema({
    windowLabel: stringValue('Window label from window.list; default main'),
    x: boundedInteger(0, 100_000, 'Left edge in CSS pixels from the WebView viewport'),
    y: boundedInteger(0, 100_000, 'Top edge in CSS pixels from the WebView viewport'),
    width: boundedInteger(1, 100_000, 'Region width in CSS pixels'),
    height: boundedInteger(1, 100_000, 'Region height in CSS pixels'),
    maxSize: boundedInteger(
      256,
      4_096,
      'Maximum output width or height in pixels; default 2048',
  ...
  'view.capture_element': objectSchema({
    windowLabel: stringValue('Window label from window.list; default main'),
    selector: nonEmptyString('Exact semantic selector returned by window.ui_snapshot'),
  ...
  'sprite.list': objectSchema({
    search: stringValue('Case-insensitive id, name, path, or slice-name substring'),
    folder: nonEmptyString('Assets folder prefix'),
    limit: boundedInteger(1, 5_000, 'Maximum sprites; default 1000'),
    offset: boundedInteger(0, 1_000_000, 'Zero-based sprite cursor; default 0'),
...
const QUERY_SUMMARIES: QuerySummary[] = [
  ...
  { id: 'scene.snapshot', category: 'scene', description: 'Read the complete active scene snapshot; Play and Pause return the live runtime clone', readOnly: true },
  { id: 'scene.authored_snapshot', category: 'scene', description: 'Read the untouched authored scene while Play or Pause may expose a different active runtime clone', readOnly: true },
  { id: 'scene.diff', category: 'scene', description: 'Read incremental active-scene changes since a revision', readOnly: true },
