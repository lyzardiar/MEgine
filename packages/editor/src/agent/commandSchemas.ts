...
import {
  FIGMA_IMPORT_COMMAND_PARAMS_SCHEMA,
  FIGMA_SETTINGS_PARAMS_SCHEMA,
} from './figmaAgentSchemas.ts';
...
export const COMMAND_PARAMS_SCHEMAS: Record<string, AgentJsonSchema> = {
  ...
  'effekseer.compose': objectSchema({
    name: {
      ...stringValue('Composition root name'),
      minLength: 1,
      maxLength: 80,
    },
    mode: {
      type: 'string',
      enum: ['world', 'screen'],
      description: 'World-space Transform composition or Canvas-hosted RectTransform composition',
    },
    parent: entityId('Optional existing parent; screen mode requires Canvas or RectTransform'),
    layers: {
      type: 'array',
      minItems: 1,
      maxItems: 16,
      items: objectSchema({
        effect: {
          ...
          maxLength: 512,
          description: 'Exact indexed .efk or .efkefc asset returned by effekseer.catalog',
        },
        name: { ...stringValue('Optional layer entity name'), maxLength: 80 },
        position: finiteTuple(3, 'World-space local position'),
        rotation: finiteTuple(4, 'World-space local rotation quaternion'),
        scale: finiteTuple(3, 'World-space local scale'),
        anchoredPosition: finiteTuple(2, 'Canvas-local anchored position in reference pixels'),
        size: finiteTuple(2, 'Canvas layer RectTransform size in reference pixels'),
        speed: { type: 'number', minimum: 0.05, maximum: 8 },
        startFrame: { type: 'integer', minimum: 0, maximum: 100000 },
        prewarm: booleanValue('Start from an already-simulated frame'),
        looping: booleanValue('Loop playback; default true'),
  ...
  'figma.settings.set': FIGMA_SETTINGS_PARAMS_SCHEMA,
