...
const KIND_SPAWNERS: Record<TypedEntityKind, (store: EditorStore) => void> = {
  ...
  particle_2d_fire: (store) => store.spawnParticleEmitter2DPreset('fire'),
  particle_2d_smoke: (store) => store.spawnParticleEmitter2DPreset('smoke'),
  particle_2d_spark_burst: (store) => store.spawnParticleEmitter2DPreset('spark_burst'),
  particle_2d_magic_aura: (store) => store.spawnParticleEmitter2DPreset('magic_aura'),
  particle_2d_snow: (store) => store.spawnParticleEmitter2DPreset('snow'),
  trail_2d: (store) => store.spawnTrailRenderer2D(),
...
const COMMAND_SUMMARIES: CommandSummary[] = [
  ...
  { id: 'profiler.clear', category: 'profiler', description: 'Clear Scene, Game, and Timeline editor-profiler data across all editor windows', readOnly: false },
  { id: 'project.open', category: 'project', description: 'Open a project from the welcome page; NOT_READY with reason=dialog requires dialog.respond then project.state', readOnly: false },
