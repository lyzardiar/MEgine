//! Rapier-backed 2D and 3D rigid-body simulation synchronized with MEngine ECS components.
...
pub struct PhysicsStepEvents {
    /// Non-sensor contact pairs that started during this step.
    pub started: Vec<CollisionPair>,
    /// Non-sensor contact pairs that stopped during this step.
    pub stopped: Vec<CollisionPair>,
    /// Sensor intersection pairs that started during this step.
    pub trigger_started: Vec<CollisionPair>,
    /// Sensor intersection pairs that stopped during this step.
    pub trigger_stopped: Vec<CollisionPair>,
}
...
fn pair_transitions(
    ...
    active: &mut HashSet<CollisionPair>,
) -> (Vec<CollisionPair>, Vec<CollisionPair>) {
    let mut started: Vec<_> = current.difference(active).copied().collect();
    let mut stopped: Vec<_> = active.difference(current).copied().collect();
