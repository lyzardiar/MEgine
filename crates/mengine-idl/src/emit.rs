...
pub fn emit_rust(defs: &[Def]) -> String {
    ...
    out.push_str("//! Author: MiYu\n//! AUTO-GENERATED by mengine-idl. Do not edit by hand.\n");
    out.push_str("#![allow(dead_code)]\n\n");
    out.push_str("use crate::component::{Component, ComponentBox};\n");
    ...
    out.push_str("pub fn component_from_value(\n");
    out.push_str("    name: &str,\n");
    out.push_str("    value: serde_json::Value,\n");
    out.push_str(") -> Result<Option<ComponentBox>, serde_json::Error> {\n");
...
fn is_language_default(ty: &str, value: &str) -> bool {
    ...
    match ty {
        ...
        "string_array" | "float2_array" | "float4_array" => compact == "[]",
        _ => false,
    }
}
...
fn default_expr(ty: &str, default: Option<&str>, optional: bool) -> String {
    ...
    if let Some(d) = default {
        return match ty {
            ...
            "string_array" | "float2_array" | "float4_array" => "Vec::new()".into(),
...
fn map_ts_type(ty: &str) -> &'static str {
    match ty {
        ...
        "float4_array" => "[number, number, number, number][]",
...
fn json_type(ty: &str) -> serde_json::Value {
    ...
    match ty {
        ...
        "float2_array" | "float4_array" => {
            let size = if ty == "float2_array" { 2 } else { 4 };
            m.insert("type".into(), "array".into());
            m.insert(
                "items".into(),
                serde_json::json!({
...
mod tests {
    ...
    #[test]
    fn emits_typed_float4_arrays_for_all_targets() {
        let defs =
            parse_idl("component MaterialPropertyBlock {\nvalues: float4_array = []\n}").unwrap();
        let rust = emit_rust(&defs);
        let typescript = emit_typescript(&defs);
        let schema = emit_json_schema(&defs);
        assert!(rust.contains("pub values: Vec<[f32; 4]>"));
    ...
    #[test]
    fn emits_one_type_erased_deserializer_for_every_component() {
        let defs = parse_idl(
            "component First { value: float = 1 }\ncomponent Second { enabled: bool = true }",
        )
        .unwrap();
        let rust = emit_rust(&defs);
