//! Opt-in campaign acceptance through the real Boa bridge and native Play worker.
...
fn complete_thunder_campaign_in_native_play_runtime() {
    ...
    let report = serde_json::json!({"passed":true,"path":"EditorPlayRuntime / Boa / native World / input-only replay","elapsedMs":start.elapsed().as_millis(),"flight":state,"phasePeaks":phase_peaks});
