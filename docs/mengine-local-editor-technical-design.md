...
- 当前 Runtime 仍使用内置 5×7 位图字形，Editor 使用系统字体近似；Font 资产与 Rich Text 尚未纳入本批，因此该批完成的是基础段落与溢出契约，不宣称 Unity Text 已全部等价。
...
- 字形数量和每帧 65,536 图元预算保持不变；Bold 扩宽现有像素矩形而不是无界复制，Italic 只改变有限坐标。无窗口回归覆盖默认值、四值枚举、Agent schema、后台 Canvas draw plan、实际 Editor font 字符串、样式宽度、Best Fit 缩小以及 Runtime 字形坐标和宽度。Editor 仍使用系统字体近似、Runtime 仍使用 5×7 位图，Font 资产与 Rich Text 继续作为独立缺口。
