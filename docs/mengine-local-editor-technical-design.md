...
当前 PC Build 为了保持脚本动态加载兼容性，仍复制完整 `Assets` 树。因此这一阶段完成的是“运行实体与 Prefab 节点剔除”，不是“未引用资源文件裁剪”。成熟发布管线仍需增加 Referenced Only 模式、Always Include/Addressable 显式白名单、资源包分组和可定位的剔除报告。
