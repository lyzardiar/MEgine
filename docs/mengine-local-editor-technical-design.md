...
## 139. 2026-07-20 Sequencer 工作区间标尺手柄

- Edit Mode 标尺新增方形零圆角的 In/Out 手柄，端点使用以时间点居中的 12px 命中区；位于资产首尾时改为向时间轴内部展开，仍保留完整 12px 可抓区域。In 占标尺上半区、Out 占下半区，即使 60 FPS 的最小一帧在低缩放下只有约 1.2px，两者也不会相互遮挡。拖动采用 Pointer Capture，指针离开 Dock 后仍可连续调整；手柄、蓝色区间带和工具栏数值框共享即时状态，拖动期间 Transport 停止，播放头若落到新区间外则夹取到最近边界。
- 单侧调整抽成无 DOM 的 `resizeSequencerPreviewRange`：拖 In 固定 Out、拖 Out 固定 In，按资产 FPS 吸附并始终保留至少一帧；`1.05s@10fps` 等非整帧尾端仍以真实 Duration 为硬上限。工具栏 In/Out 数值框也改走同一逻辑，不再出现鼠标拖动固定对侧、手输越界却推动对侧的两套行为。
- 两个手柄提供 Slider 语义、当前秒数与精确可达的最小/最大值；获得焦点后可用左右方向键逐帧调整、`Shift` 加速为十帧、`Home/End` 到边界。聚焦端点会按 12px 边距把对应时间重新带回有效轨道视口，计算会扣除 180px Sticky Track Header；标尺改用不可滚动的 `overflow: clip`，避免浏览器为了聚焦边缘手柄而单独改变 Ruler `scrollLeft`、造成刻度与轨道错位。键盘事件不会冒泡为 Timeline 播放头逐帧移动，鼠标手柄事件也不会落入标尺 Scrub，因而一次手势只改变一个目标。

第一遍自省从状态入口一致性反查：初稿只给拖拽调用单侧 Resize，而原有 In/Out 数字输入继续调用通用 Normalize；当输入越过另一端时，Normalize 会同时移动两个端点。现将拖拽、键盘和数值框统一到单侧函数，并从函数结果计算 HTML/ARIA 边界，测试覆盖吸附、交叉夹取、固定对侧、非整帧尾端与非有限输入。
...
## 156. 2026-08-01 Control Source 激活与 Post Playback

- 非 Prefab Control Clip 新增显式 `control_activation` 与 `post_playback`。新建 Clip 默认启用 Control Activation，Post Playback 默认 Revert；旧资产缺少开关时按关闭处理，以保留此前不接管场景对象激活状态的行为，首次保存后会写出明确值。Post Playback 只接受 Active、Inactive、Revert 三态，与 Activation Track 独有的 Leave As Is 分开校验。Editor、Rust 资产层和 CLI 最终打包审计共享类型与白名单，错误布尔值或未知终态无法进入 Player。
- Control Track 的 Target 现在在 Inspector 中明确标为 `Source / Prefab Parent`。没有 Prefab 时，它是受 Control Activation 管理的场景对象：Director 生命周期开始后，Clip 外空隙为非激活、Clip 内为激活；Pause 保留当前采样，显式 Stop、正反向自然结束和 Director 失活才应用 Post Playback。Mute、资源热改、绑定失效或目标改变走普通 Revert，不把配置变化误当播放完成。激活快照复用既有稳定 binding、作者初始状态和 sibling 保持逻辑；嵌套 Control 仍受 `track-id:clip-index/` 生命周期作用域约束，父 Clip 结束时会完成子 Source 的终态。
- Prefab Clip 继续把同一 Target 解释为 Parent，不会激活或停用作者父对象；Prefab 实例自身仍由上一批生命周期在 Clip 内激活、空隙停用、Director 结束销毁。编辑态 Scene Preview 可在克隆快照中准确显示非 Prefab Source 的 Clip 内/空隙激活结果，不修改作者场景；静态预览仍不伪造 Prefab 临时实体。Sequencer 仅在非 Prefab Clip 显示 Control Activation 和三态 Post Playback，Agent 可在隐藏窗口完整读取并语义写入。
...
## 169. 2026-08-01 GraphicRaycaster 反向图形与物理遮挡
...
## 198. 2026-08-01 Unity Graphic Material 与 UI Shader

- `Image`、`RawImage`、`Text` 与 `Panel` 新增共享的 `material` 资产槽，默认值为空并继续使用内置 UI Shader。IDL、生成的 Rust/TypeScript/JSON Schema、组件目录、标准 UI 工厂、Inspector 项目资产选择器和 Agent component schema 使用同一字段；Image 的专用 Inspector 同样提供可拖放、可选择、可清空的 Material 槽，不会退化成自由字符串输入。Editor Canvas 的批次快照公开规范化材质路径并按材质隔离，后台 Agent 可直接观察。Sliced、Tiled、Filled、文字轮廓和 Panel Border 产生的全部 Runtime 图元都会继承所属 Graphic 的替换材质，但仍保留 Graphic 自己的 Sprite/Texture 作为主纹理及其顶点颜色、UV、裁剪、Stencil 和额外 Shader Channels。
- `.mmat`/`.mat`/`.minst` 与 `.mshader` 继续复用现有材质继承、参数、Keyword、四路自定义纹理、颜色空间和采样器契约；UI Shader 必须实现独占的 `mengine_ui_hook(MEngineUiInput)`，不能与 `mengine_surface_hook` 或 `mengine_lit_surface_hook` 混写。输入公开 Vertex Color、UV0..3、Normal、Tangent、Screen Position、Channel Mask 和 Instance Index，并提供主纹理、材质颜色、反射参数、Keyword 与自定义纹理 helper。错误域、非法 WGSL、缺失纹理或失效绑定进入可见的洋红错误管线，而不是静默回退成内置材质。
- Runtime 在 Sprite 子资源解析后、最终合批前解析 UI 材质，使 Blend Mode 与 Shader identity 成为真实批次键；每帧共享不可变材质实例，参数进入 per-primitive storage buffer，自定义纹理进入独立四纹理 Bind Group，主纹理仍按 Graphic 批次绑定。Alpha、Premultiplied、Additive 和 Multiply 都有对应 GPU Blend State；内置、Custom 与 Error 管线共享 Depth/Stencil/Soft Clip 路径。无窗口 GPU 回归会在 headless adapter 上实际组合、验证、创建并编码自定义 UI 管线。
- PC Build 依赖扫描会从四类 Graphic 收集材质、父实例、UI Shader 与反射纹理，并把 Shader variant 标记为 `surface` 或 `ui`。同一材质被错误用于两个域、UI Graphic 引用 Surface Shader、MeshRenderer 引用 UI Shader都会在发布前失败；UI variant 强制关闭 Depth Write/Double Sided 并拒绝 Replace Blend。`--validate-package` 会逐场景重新解析 UI 材质、Shader 与纹理，因此编辑器可用但发布包漏资源不再被视为成功。当前 TypeScript Canvas 预览公开材质身份但仍用内置 2D 着色近似，只有原生 Game/Player RHI 执行 WGSL；该能力也不包含 Unity 的运行时 `materialForRendering` modifier 链、CanvasRenderer 多材质槽或外部自定义 Mesh API，后续需在对应数据模型存在后单独实现。

## 199. 2026-08-01 Unity Text 段落换行与溢出
...
- 当前 Runtime 仍使用内置 5×7 位图字形，Editor 使用系统字体近似；Font 资产尚未纳入本批，因此该批完成的是基础段落与溢出契约，不宣称 Unity Text 已全部等价。
...
- 字形数量和每帧 65,536 图元预算保持不变；Bold 扩宽现有像素矩形而不是无界复制，Italic 只改变有限坐标。无窗口回归覆盖默认值、四值枚举、Agent schema、后台 Canvas draw plan、实际 Editor font 字符串、样式宽度、Best Fit 缩小以及 Runtime 字形坐标和宽度。Editor 仍使用系统字体近似、Runtime 仍使用 5×7 位图，Font 资产继续作为独立缺口。

## 202. 2026-08-01 Unity Text Align By Geometry

- `Text.align_by_geometry` 对齐 Unity `Text.alignByGeometry` authoring 契约，默认 `false`。IDL、生成的 Rust/TypeScript/JSON Schema、组件目录、标准 Text 工厂、Inspector 与 Agent schema 共享同一字段，后台 Canvas draw plan 也显式暴露最终取值。
- 关闭时沿用字形 advance metrics；开启时 Left/Center/Right 改用每行可见字形的左右几何边界，因此前导/尾随空格与 `1`、`.`、`:` 等窄字形不会把可见内容错误推离目标边缘。换行、Best Fit 和逐行宽度仍按 advance metrics 求解，避免对齐开关改变排版换行，这是 Unity 该选项只影响水平对齐的边界。
- Editor 不再把水平对齐委托给浏览器字体锚点：实际 Canvas 优先使用当前系统字体的 `TextMetrics.actualBoundingBoxLeft/Right`，不提供该度量的后台测试环境则使用同一份确定性 5×7 几何回退。Runtime 逐行扫描真正点亮的位图像素并包含每一行实际的 Bold 扩宽与 Italic 斜切，而不是仅对少数字符写死边界。无窗口回归覆盖旧场景默认值、Agent schema、后台 draw plan、Editor 实际 `fillText` 坐标、Runtime 窄字形、组合样式与前导空格。当前系统字体和内置位图的具体字形轮廓仍是近似，Font 资产作为后续独立缺口。

## 203. 2026-08-01 Unity Text Rich Text
