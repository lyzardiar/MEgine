...
当前落地的是“同步层”第一阶段：附加层共享 Base State Machine，Avatar Mask 以内嵌路径配置保存。独立层状态机、独立层参数/权重的运行时脚本控制、外部可复用 `.mavatar` 资产、Humanoid Body Mask、IK Pass、层级动画事件与 Root Motion 合成仍需后续实现；编辑器本轮通过类型检查与构建验证，未在本轮重新启动浏览器做视觉验收。
