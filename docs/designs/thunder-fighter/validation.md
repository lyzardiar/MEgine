...
- `native-campaign.json`：输入回放经真实 QuickJS 和原生 World 约 109 秒完成，五种 Boss 阶段均经过；峰值 478 发敌弹、56 次击落、16 次擦弹、4 次 Nova、1 次受伤。
...
2026-09-26 性能阶段：脚本迁移至 QuickJS-NG，弹幕通过原生数值数组接口提交。资源变更检查按 250 ms 轮询，原生完整通关再次通过。最终 Release 实战分项耗时、现场截图、编辑器回归和最新 Player 哈希见[性能报告](../editor-performance/README.md)。
