...
| `window.ui_press_key` | `{ windowLabel?, selector, key, shiftKey?, ctrlKey?, altKey?, metaKey? }` | ✅ 允许 Enter/Escape/Tab/Space、方向/翻页/首尾/删除类语义键、F1–F24 功能键，或单个非空白可打印 Unicode 字符；可打印键通过当前 React props 同步受控草稿，只读控件仍可聚焦、选择和导航但绝不修改值或报告提交。Enter/Escape 只为可编辑控件补齐失落的 blur 提交，Tab 对可编辑草稿先提交再按原生 HTML/SVG 语义焦点顺序移动、对按钮/菜单项和只读控件则仅移动焦点；功能键与修饰键组合可触发编辑器快捷键。多字符文本、控制字符与调用方脚本仍被拒绝，所有事件只进入目标隐藏 WebView，不向前台应用注入输入 |
