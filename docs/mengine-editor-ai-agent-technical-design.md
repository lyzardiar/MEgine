...
| 适配器：MCP stdio 洪泛或宿主停止读取 | sidecar 输入逐字节累计且单行最多 64 MiB；最多 128 个活动 MCP 请求和 64 个在途 Bridge RPC。stdout 响应按序写入、64 MiB 时暂停 stdin，合计 192 MiB 时终止失去消费能力的会话，避免 Node 内部流缓冲无限增长 |
