...
| `window.ui_press_key` | `{ windowLabel?, selector, key, shiftKey?, ctrlKey?, altKey?, metaKey? }` | ✅ 允许 Enter/Escape/Tab/Space、方向/翻页/首尾/删除类语义键、F1–F24 功能键，或单个非空白可打印 Unicode 字符；可打印键通过当前 React props 同步受控草稿，只读控件仍可聚焦、选择和导航但绝不修改值或报告提交。Ctrl+A/Meta+A 对非密码 input、textarea 与 contenteditable 执行原生等价全选；Ctrl/Meta+C/X/V 使用仅存在于目标 WebView 的 Agent 私有文本剪贴板完成复制、剪切和粘贴，回执公开操作、长度与 `window-private` 范围，绝不读取或污染操作系统剪贴板。密码框拒绝私有剪贴板访问，只读控件不被剪切或粘贴；跨窗口传递使用 `window.ui_content` 精确读取加 `window.ui_set_value`，不会把文本持久化到共享存储。普通左右移动、纵向落点与向前/向后删除均使用 Unicode grapheme 边界，不会拆坏 emoji、组合音标或 ZWJ 连接字符，并为旧 WebView 保留 code-point 安全回退。Ctrl/Meta+Left/Right 在 Unicode 单词起点之间移动或扩选，Ctrl/Meta+Backspace/Delete 删除完整单词跨度，Ctrl/Meta+Home/End 跳到全文首尾。Enter/Escape 只为可编辑控件补齐失落的 blur 提交，Tab 对可编辑草稿先提交再按原生 HTML/SVG 语义焦点顺序移动、对按钮/菜单项和只读控件则仅移动焦点；功能键与修饰键组合可触发编辑器快捷键。多字符文本、控制字符与调用方脚本仍被拒绝，所有事件只进入目标隐藏 WebView，不向前台应用注入输入 |
