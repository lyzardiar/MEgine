...
| 多窗口同步 | 按原生编辑器实例 ID 隔离的 BroadcastChannel（Workspace / Panel / Asset / Scene/Game Profiler / Timeline Profiler / Sorting Layers / Dialog） | 全量状态复制协议，可作事件流参考；多个同源编辑器进程不会串扰 |
...
MCP 另提供 `take_all_window_screenshots`：先后两次读取原生窗口清单，中间串行调用离屏整窗截图，并把每个成功窗口作为独立 image content block 返回。结果显式给出 `inventoryStable`、`complete`、每窗元数据/错误和图像索引；单次最多 32 个窗口、每窗最长边 2048，聚合 Base64 达到 32 MiB 时停止继续编码并要求降低 `maxSize`，不会用无界多图结果挤爆 MCP 输出队列。该工具只读取 WebView2 渲染面，不显示、激活或聚焦任何窗口。当前源码以真实 MCP `tools/call` 在后台实例中一次抓取主窗、15 个分离面板和 2 个注册扩展窗，18/18 个 image block 返回成功、清单稳定且零失败，抓取前后所有窗口均为 `visible=false/focused=false`。
...
| `window.ui_hover` | `{ windowLabel?, selector, state?, offsetX?, offsetY? }` | ✅ 接受快照标记为 `hover` 的可点击控件或 React 悬停目标；`state=enter`（默认）通过 WebView2 DevTools 虚拟指针触发真实 CSS `:hover` 与 React 指针生命周期，`state=leave` 显式释放 hover 并收起临时 UI，整个过程不移动系统鼠标或激活窗口 |
| `window.ui_press_key` | `{ windowLabel?, selector, key, shiftKey?, ctrlKey?, altKey?, metaKey? }` | ✅ 允许 Enter/Escape/Tab/Space、方向/翻页/首尾/删除类语义键、F1–F24 功能键，或单个非空白可打印 Unicode 字符；可打印键通过当前 React props 同步受控草稿，只读控件仍可聚焦、选择和导航但绝不修改值或报告提交。Ctrl+A/Meta+A 对非密码 input、textarea 与 contenteditable 执行原生等价全选；Ctrl/Meta+C/X/V 使用仅存在于目标 WebView 的 Agent 私有文本剪贴板完成复制、剪切和粘贴，回执公开操作、长度与 `window-private` 范围，绝不读取或污染操作系统剪贴板。Ctrl/Meta+Z 撤销、Ctrl/Meta+Y 或 Ctrl/Meta+Shift+Z 重做 Agent 键盘文本编辑；历史以 `WeakMap` 按元素隔离，仅保留 64 个快照且总计不超过 100 万字符，回执公开操作、是否应用、栈深与 `element-private` 范围，绝不进入场景、资源或文档的全局 Undo 栈。`window.ui_set_value` 建立新的确定性基线并清除该元素旧历史。密码框拒绝私有剪贴板与文本历史访问，只读控件不被剪切、粘贴、撤销或重做；跨窗口传递使用 `window.ui_content` 精确读取加 `window.ui_set_value`，不会把文本持久化到共享存储。普通左右移动、纵向落点与向前/向后删除均使用 Unicode grapheme 边界，不会拆坏 emoji、组合音标或 ZWJ 连接字符，并为旧 WebView 保留 code-point 安全回退。Ctrl/Meta+Left/Right 在 Unicode 单词起点之间移动或扩选，Ctrl/Meta+Backspace/Delete 删除完整单词跨度，Ctrl/Meta+Home/End 跳到全文首尾。Enter/Escape 只为可编辑控件补齐失落的 blur 提交，Tab 对可编辑草稿先提交再按原生 HTML/SVG 语义焦点顺序移动、对按钮/菜单项和只读控件则仅移动焦点；功能键与修饰键组合可触发编辑器快捷键。多字符文本、控制字符与调用方脚本仍被拒绝，所有事件只进入目标隐藏 WebView，不向前台应用注入输入 |
...
快照版本 24 收紧 `keyPress` 与只读能力声明：只有原生可聚焦控件、contenteditable、显式 `tabindex`（含 `-1`）或可聚焦 SVG 才公开按键动作。仅在父容器监听冒泡键盘事件、但自身不能获得焦点的 menu/dialog 包装层不再产生无法执行的虚假动作；`readOnly` 或 `aria-readonly=true` 的输入、文本区、选择框与 contenteditable 不公开 `setValue`，键盘执行也不会通过字符、删除、步进或选项导航绕过只读边界。Agent 应把按键发送给快照中的实际焦点项，事件仍会按 DOM/React 规则冒泡到容器处理。
...
Inspector 的每个组件块同时声明组件标题为 `data-agent-scope`。同名字段会得到 `Image / Alpha`、`Button / Label` 这类组件级 `qualifiedName`，不再全部退化成通用的 `Inspector / Property` 范围；组件上下文菜单、专用编辑器与通用 Schema 字段使用同一消歧边界。
