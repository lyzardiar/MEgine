...
编辑器中的图形表面也必须声明机器可判定的语义类型：Scene/Game 及可编辑曲线使用 `application`，Animator 图使用保留子控件的 `group`，Profiler、只读曲线、Sprite 与 Sprite Atlas 预览使用 `img`。因此 Agent 能先从快照区分“交互式画布、图形控件组、只读图像”，再按需使用 `view.capture_element` 读取对应像素，而不必仅凭 `canvas`/`svg` 标签猜测用途。Profiler 的底层采样与 `profiler.summary` 查询始终保持实时；仅当编辑器 WebView 未聚焦时，Profiler 面板把 React DOM 刷新合并到每秒一次，为 revision-safe 的精确内容读取、元素截图和语义写操作保留稳定执行窗口，重新聚焦后立即恢复逐样本 UI 刷新。
