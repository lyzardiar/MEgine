...
| `editor.state` | ✅ `{ mode, gizmo, sceneCamera, sceneView: { mode2D, gridVisible, smartGuidesEnabled, snap }, gameResolution, canUndo, canRedo, undoLabel, redoLabel, dirty, sceneName, sceneRevision, eventSequence }`；Scene 编辑开关与吸附参数在同一编辑器实例的所有窗口间持久化同步 |
...
| `view.set_scene_preferences` | `{ mode2D?, gridVisible?, smartGuidesEnabled?, snap?: { enabled?, move?, rotate?, scale? } }` | ✅ 精确部分更新并持久化 Scene 编辑开关；同实例窗口实时同步并触发 `view.changed`，省略字段保持不变 |
...
| 事件订阅 | ✅ 有界 journal + cursor 查询 `events.get`；`events.wait` 可按 topic 等待最多 15 秒并显式返回 `timedOut`，MCP/CLI 无需高频轮询；最多 64 个等待请求，超限返回带容量与重试提示的 `RATE_LIMITED`；MCP `notifications/cancelled` 会按原请求 ID 精确释放 sidecar、原生 Bridge 和 WebView waiter 三层资源且不回写已取消响应，不会关闭共享 WS 或误伤其他请求；原生 WebSocket 同时广播 `project.changed` / `scene.changed` / `selection.changed` / `mode.changed` / `dialog.changed` / `log.*` / `panel.changed` / `workspace.changed` / `window.changed` / `window.types.changed` / `menu.changed` / `view.changed` / `build.progress` / `build.artifacts` / `build.settings` / `project.settings` / `asset.changed`。其中 `dialog.changed` 覆盖所有窗口非阻塞对话框的打开、切换和关闭，`workspace.changed` 覆盖资源文档路径、dirty、激活状态和宿主窗口变化，`window.changed` 由完整原生窗口清单去重驱动，覆盖 Agent 或用户路径造成的创建、关闭、显隐、焦点、位置与尺寸变化，并在窗口消失后撤销 Agent 所有权，`window.types.changed` 覆盖可创建辅助窗口类型注册变化，`menu.changed` 覆盖菜单注册及实时可用性变化，`view.changed` 覆盖 Scene 相机、Gizmo、Game 分辨率，以及任意窗口修改的 2D、网格、智能参考线与吸附参数，`build.artifacts` 只在构建历史、补丁或发布产物确实变化时发出，避免用高频 progress 失效库存资源，`build.settings` 与 `project.settings` 分别统一覆盖 UI 或 Agent 在任意窗口成功保存构建策略，以及 Tags、GameObject Layers 与 Sorting Layers，`asset.changed` 同样覆盖 UI 或 Agent 成功重命名、删除场景以及其他资源写入；同一次保存或场景资源变更只写一条 journal 事件。MCP 另声明标准 `resources.subscribe`，把这些事件映射为已订阅资源的 `notifications/resources/updated`；同一事件循环内按 URI 合并，断线持续重连，重连成功后统一失效已有订阅以补偿断线窗口 |
