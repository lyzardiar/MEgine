...
- Brick 已移植并通过原生 Agent 验收：128 → 126 块砖、挡板移动、实体 ID 稳定、落底重开、R 重开、停止恢复、再次播放清空输入。Animated Tile、Destructible、Palette Swap 也已完成，Random Tile、Weighted Random Tile、Terrain Tile、Pipeline Tile 也已完成，Auto Tile、Custom Rule Tile、Rule Override Tile、Tint Brush、Tint Brush Smooth 也已完成，合计 13/20；其余 7 项尚未完成。
...
- 颜色：原生后处理支持 `EnvironmentLight.tone_mapping=false`，默认仍启用 ACES。十三个 Gamma 来源项目在线性渲染前转换数值颜色；原生截图采样背景为 (49,77,121)，与源颜色相符。每个 QA 入口均检查背景颜色。
